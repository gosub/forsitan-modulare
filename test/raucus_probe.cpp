...
//
// Usage: ./raucus_probe [tone|diode|level|alias]
...
static void probeLevel() {
    ...
    for (float inV : {0.2f, 1.f, 5.f, 10.f}) {
        for (float sus : {0.f, 0.5f, 1.f}) {
            Raucus m;
            long fr = 0;
            m.params[Raucus::SUSTAIN_PARAM].setValue(sus);
            m.params[Raucus::VOLUME_PARAM].setValue(1.f);
            ...
            for (int n = 0; n < (int)(0.5 * SR) + kN; n++) {
                ...
                if (k >= 0) out[k] = m.outputs[Raucus::AUDIO_OUTPUT].getVoltage();
            }
...
static void probeAlias() {
    ...
    for (int cb : {19, 191}) {                    // 222.7 Hz and 2238 Hz
        ...
        for (int os = 4; os >= 0; os--) {
            ...
            long fr = 0;
            m.osIndex = os;
            m.params[Raucus::SUSTAIN_PARAM].setValue(1.f);
            m.params[Raucus::VOLUME_PARAM].setValue(1.f);
            m.inputs[Raucus::AUDIO_INPUT].channels = 1;
            std::vector<double> out(kN);
            for (int n = 0; n < (int)(0.5 * SR) + kN; n++) {
