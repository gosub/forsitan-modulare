...
- **Title**: module name in **OCR-A**, baked to SVG `<path>` (fill `#f9f9f9`),
  ...
  **The title cap height is 3.2mm on every panel, whatever its width**
  ...
  smaller title: its column headers start at y≈8.8.

  A title level with the screws has to fit *between* them, and on a narrow
  panel a long name does not: at 3.2mm "viginti" is 22.8mm wide against the
  16.4mm an 8HP panel leaves. The fix is not a smaller title but a lower one.
  `titley=` moves the baseline down, and a title whose top clears the screw
  band (y=6.1) is measured against the full panel width instead:

  ```
  // @layout:begin viginti 40.64 128.5 titley=10.0
  ```

  `viginti` is the one panel using it. A long name with no `titley` still
  shrinks automatically to clear the screw zones (only MMCCCXCIX does now,
